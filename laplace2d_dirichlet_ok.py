...
#   -               -
#   A---------------B
bc1 = np.zeros(Np) #[xa,ya] - [xb, yb]
bc2 = np.zeros(Np) #[xb,yb] - [xc, yc]
bc3 = np.sin(x3) #[xc,yc] - [xd, yd]
bc4 = np.zeros(Np) #[xd,yd] - [xa, ya]
...
bci = np.concatenate((bc1, bc2[1:len(bc1)], bc3[1:len(bc3)],bc4[1:len(bc4)]))
xindex = np.linspace(0,1, len(bci))
...
if (plotBC == 1):
    plt.plot(xindex, bci, xindex, bci, 'bo')
...
if (plotSolution==1):
    ...
    ax.plot_surface(xx, yy, uanal, cmap=cm.coolwarm,
...
sizeA = len(xcolloc)
Abij = np.zeros((sizeA, sizeA))
Bbij = np.zeros((sizeA, sizeA))
...
for ii in range(0, sizeA): #for each collocation point
    for jj in range(0, sizeA): #we evaluate the effect of source distribution on the panels
        ...
        Abij[ii][jj] = constantStrengthSource(x1, y1, x2, y2, xp, yp) #w
        Bbij[ii][jj] = constantStrengthDoublet(x1, y1, x2, y2, xp, yp)#dw/dn
        deltaKronecker = 0
        if (ii == jj):
            deltaKronecker = 1
    ...
    udirichlet[ii][0] = bci[ii]

b = np.matmul(A, udirichlet)
ubem = np.linalg.solve(S,b)

# comparison
xindex = np.linspace(0, 1, len(xcolloc))
plt.plot(xindex, udirichlet)
plt.plot(xindex, udirichlet, 'bo', label='dirichlet data')
...
#comparison of the analytic solution and the numerical from bem
...
for kk in range(0, len(xtest)):
    for ll in range(0, len(ytest)):
        ...
        for jj in range(0, len(xcolloc)):
            x1, y1 = xi[jj], yi[jj]
            x2, y2 = xi[jj+1], yi[jj+1]
            SL = constantStrengthSource(x1, y1, x2, y2, xp, yp) #w
            DL = constantStrengthDoublet(x1, y1, x2, y2, xp, yp)#dw/dn
            utemporary = utemporary + ubem[jj][0]*SL - udirichlet[jj][0]*DL
...
ax.set_zlabel('Z axis')
plt.show()
# BEWARE THE PLOT IS TITLED??? WHAT THE ACTUAL FUCK
